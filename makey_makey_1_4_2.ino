 ...
 ****************
  ...
  ////////////////////////////////////////////////
  MIT Media Lab & Sparkfunwasdf
  stwart date: 2/16/2012
...
// Pin Numbers
// input pin numbers for kickstarter production boardw
...
void setup()
{
  ...
  delay(2000);
  lcd.clear();
  lcd.print(" ADELIA PUTRI S"); // You can make spaces using well... spaces
  lcd.setCursor(3, 1); // Or setting the cursor in the desired position.
  lcd.print("1110191025");
  delay(2000);
  lcd.clear();
  initializeArduino();
  initializeInputs();
...
void initializeInputs()
{
  ...
  for (int i = 0; i < NUM_INPUTS; i++) {
    ...
    Serial.println("Command : " + String(inputs[i].keyCode));
#ifdef DEBUG
    Serial.println(i);
#endif
